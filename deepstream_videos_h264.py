...
def get_source(url, index, pipeline):
    # Source element for reading from the file
    print("Creating Source \n ")
    source = Gst.ElementFactory.make("filesrc", "file-source-" + str(index))
    if not source:
    ...
    h264parser = Gst.ElementFactory.make("h264parse", "h264-parser" + str(index))
    if not h264parser:
        sys.stderr.write(" Unable to create h264 parser \n")

    # Use nvdec_h264 for hardware accelerated decode on GPU
    print("Creating Decoder \n")
    decoder = Gst.ElementFactory.make("nvv4l2decoder", "nvv4l2-decoder" + str(index))
    if not decoder:
...
def main(rtsps, width, height, tiler_rows, tiler_columns, TILED_OUTPUT_WIDTH, TILED_OUTPUT_HEIGHT, config_file_path):
    print(rtsps)

    for i in range(len(rtsps)):
        str_key = "stream{0}".format(i)
        fps_streams[str_key] = GETFPS(i)
    ...
    for i in range(len(rtsps)):
        url = rtsps[i]
        decoder = get_source(url, i, pipeline)
        sinkpad = streammux.get_request_pad("sink_" + str(i))
        if not sinkpad:
    ...
    streammux.set_property('batch-size', len(rtsps))
...
if __name__ == '__main__':
    videos = []
    videos.append('/opt/nvidia/deepstream/deepstream/samples/streams/sample_720p.h264')
    videos.append('/opt/nvidia/deepstream/deepstream/samples/streams/sample_720p.h264')
    videos.append('/opt/nvidia/deepstream/deepstream/samples/streams/sample_720p.h264')
    videos.append('/opt/nvidia/deepstream/deepstream/samples/streams/sample_720p.h264')
    width = 1280
    ...
    tiler_rows = 2
    tiler_columns = 2
    TILED_OUTPUT_WIDTH = 1800
    TILED_OUTPUT_HEIGHT = 1080
    config_file_path = 'dstest3_pgie_config.txt'
    main(videos, width, height, tiler_rows, tiler_columns, TILED_OUTPUT_WIDTH, TILED_OUTPUT_HEIGHT, config_file_path)
    sys.exit()
