...
def get_source(url, index, pipeline):
    ...
    source = Gst.ElementFactory.make("rtspsrc", "rtsp-source-" + str(index))
    if not source:
        sys.stderr.write(" Unable to create Source \n")
    source.set_property("short-header", "true")
    source.set_property('latency', 0)  # 缓存毫秒
    depay = Gst.ElementFactory.make('rtph264depay', "depay" + str(index))
    if not depay:
...
if __name__ == '__main__':
    ...
    main(rstps, width, height, tiler_rows, tiler_columns, TILED_OUTPUT_WIDTH, TILED_OUTPUT_HEIGHT, config_file_path)
    sys.exit()
